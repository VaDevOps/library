...
func JenkinsMultiBranch(username string,password string,jenurl string,job string,branch string,key string) error {
	client := http.Client{}

	URL := jenurl+ "/job/"+ job + "/job/" + branch + "/build?token=" + key
	req, err := http.NewRequest(http.MethodGet, URL, http.NoBody)
	if err != nil {
		return err
	}
